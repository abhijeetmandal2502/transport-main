...
const ApproveBill = ({ data }) => {
    ...
    const column = ["#", "Bill No.", "Consignor", "Total Weight", "Amount", "View", "Bill", 'Status'];
    const rows = [];
    data.map((item, i) => {
        rows.push([i + 1, item.bill_no, item.consignor, item.total_weight, item.process_amount, <AddBoxOutlined key={i} onClick={() => callApiData(item.id)} />, <Print key={i} onClick={() => PrintBill(item.id)} />, <Button key={i} size="small" variant="contained" color="secondary" onClick={() => router.push(`/account/ftl/approve-bill/${item.id}`)} >Update</Button>]);
    })
...
export async function getServerSideProps(ctx) {
    ...
    if (session !== null && session !== undefined) {
        ...
        try {
            const req = await fetch(
                `${process.env.apiUrl}/offline-invoice-status/processing`,
                {
                    headers: {
                        Accept: 'application/json',
                        'Content-Type': 'application/json',
