...
const ProcessingBill = ({ data }) => {
    ...
    const column = ["#", "Bill No.", "Consignor", "Total Weight", "Amount", "View", "Bill"];
    const rows = [];
    data.map((item, i) => {
        rows.push([i + 1, item.bill_no, item.consignor, item.total_weight, item.process_amount, <AddBoxOutlined key={i} onClick={() => callApiData(item.id)} />, <Print key={i} onClick={() => PrintBill(item.id)} />]);
    })
